...
(async () => {
    ...
    const v = db.product_variants.find(x => x.product_id === db.products[2].id && x.size === 'M');
    const mockLog = [];
    ...
    await step('seed an order through the storefront', async () => {
        ...
        await page.click('[data-size="M"]'); await page.click('#addBtn');
    ...
    await step('products: list + editor + add variant + save', async () => {
        ...
        if (rows !== 4) throw new Error('variants=' + rows);
        await page.click('#addVariant');
        const last = page.locator('#variantRows tr').last();
        await last.locator('[name=size]').fill('XXL');
        await last.locator('[name=stock]').fill('2');
        await page.fill('[name=compare_at_price]', '1200');
        ...
        const xxl = db.product_variants.find(v => v.product_id === p.id && v.size === 'XXL');
        if (!xxl || xxl.stock !== 2) throw new Error('variant not saved');
        if (Number(p.compare_at_price) !== 1200) throw new Error('compare price not saved');
    });
    ...
    await step('products: duplicate variant rejected', async () => {
        ...
        await page.waitForSelector('#productForm');
        await page.click('#addVariant');
        await page.locator('#variantRows tr').last().locator('[name=size]').fill('M');
        await page.click('#saveProduct');
    ...
    await step('products: new product with image upload', async () => {
        ...
        const rows = await page.locator('#variantRows tr').count();
        if (rows !== 4) throw new Error('sizes rows=' + rows);
        // صورة PNG صغيرة
        ...
        if (db.product_variants.filter(v => v.product_id === p.id).length !== 4) throw new Error('variants not inserted');
